...
async def upload_pdf(file: UploadFile = File(...)):
    try:
        ...
        all_documents = [doc for doc in all_documents if doc["source"] != file.filename]
...
async def delete_pdf(filename: str):
    try:
        ...
        if os.path.exists(DOCUMENTS_FILE):
            ...
            documents = [doc for doc in documents if doc["source"] != filename]
...
async def get_documents():
    documents = load_documents()
    filenames = list(set(doc["source"] for doc in documents))
...
async def ask_question(data: dict = Body(...)):
    try:
        ...
        if result is None:
            return {"success": False, "error": "Selected PDF not found"}
        chat_history.append({
            "question": question,
